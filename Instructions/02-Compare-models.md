...
1. Azure Portal 탭으로 돌아가서 배포 스크립트에서 이전에 만든 리소스 그룹으로 이동하고 **Azure AI Foundry** 리소스를 선택합니다.
1. 리소스의 **개요** 페이지에서 **엔드포인트를 보려면 여기를 클릭**을 선택하고 AI Foundry API 엔드포인트를 복사합니다.
1. 엔드포인트를 메모장에 저장합니다. 이 엔드포인트를 사용하여 클라이언트 응용 프로그램에서 프로젝트에 연결할 수 있습니다.
...
1. 코드 파일에서 **your_project_endpoint** 자리 표시자를 이전에 복사한 프로젝트의 엔드포인트로 바꿉니다. 연습에 사용된 첫 번째 및 두 번째 모델이 각각 **gpt-4o** 및 **gpt-4o-mini**인지 확인합니다.
...
1. 마지막 스크립트를 실행하기 전에 Azure Portal에서 Azure AI Foundry 리소스의 ID를 복사해야 합니다. Azure AI Foundry 리소스의 개요 페이지로 이동하여 **JSON 보기**를 선택합니다. 리소스 ID를 복사하고 코드 파일의 `your_resource_id` 자리 표시자를 바꿉니다.
